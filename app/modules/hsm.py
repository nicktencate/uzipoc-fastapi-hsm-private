    ...
    def deencrypt(self, thefunc: str, name: str, label: str, so: SearchObject):
        ...
        if objs:
            ...
            if obj.key_type==pkcs11.KeyType.RSA:
                ...
                if so.mechanism:
                    if thefunc == 'verify':
                        return toexec(data, base64.b64decode(so.signature), mechanism=getattr(pkcs11.Mechanism, so.mechanism), mechanism_param=mechanism_param)
                    return base64.b64encode(toexec(data, mechanism=getattr(pkcs11.Mechanism, so.mechanism), mechanism_param=mechanism_param))
