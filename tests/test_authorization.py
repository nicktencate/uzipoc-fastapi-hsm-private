...
def makecert(name):
    asn1pk = asn1crypto.keys.PublicKeyInfo({ "algorithm": {"algorithm": "rsa", "parameters": None}, "public_key": {'modulus': 0, 'public_exponent': 0} })
    thecert = asn1crypto.x509.Certificate({'tbs_certificate': tests.certgen.certgen(name, asn1pk, {"algorithm": 'md5_rsa'}) , 'signature_value': b'\x00' , 'signature_algorithm': {'algorithm': 'md5_rsa'}})
