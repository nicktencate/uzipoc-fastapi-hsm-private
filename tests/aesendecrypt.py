...
def test(session, baseurl):
    ...
    for mode in ['AES_CBC_PAD', 'AES_CFB8', 'AES_CFB8', 'AES_CFB64', 'AES_CFB128', 'AES_ECB', 'AES_CBC']:
        if mode in mechanisms:
            print("Testing AES mode:",mode) 
            message = message+b'\x00'*(16-len(message))   # All mechanisms tolerate 128 bit inputs
            params = {
                'label': "AESkey",
                'objtype': 'SECRET_KEY',
                'data': b64encode(message).decode(), # 128 bits = 16 bytes input required for some
                'mechanism': mode
            }
            encrypted = session.post(baseurl+"/encrypt", json = params).json()['result']
